...
static JSValue js_winproc_then_job(
    JSContext *ctx,
    int argc,
    JSValueConst *argv)
{
...
static DWORD WINAPI js_winproc_wait_thread(LPVOID arg)
{
    ...
    if (!JS_IsUndefined(pd->then_cb))
        cb = JS_DupValue(pd->ctx, pd->then_cb);
    ...
    if (!JS_IsUndefined(cb)) {
        JSValue argv[2];
        argv[0] = cb;
        argv[1] = JS_NewInt32(pd->ctx, pd->exit_code);
        JS_EnqueueJob(pd->ctx, js_winproc_then_job, 2, argv);
        JS_FreeValue(pd->ctx, cb);
        JS_FreeValue(pd->ctx, argv[1]);
    }
...
static void js_winproc_finalizer(
    JSRuntime *rt,
    JSValueConst val)
{
    ...
    JSModuleDef *m)
{
    ...
    JS_SetPropertyFunctionList(
        ctx,
        proto,
        js_winproc_proto_funcs,
        sizeof(js_winproc_proto_funcs) /
